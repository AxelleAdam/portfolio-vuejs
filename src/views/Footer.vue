<template
  ><div>
    <h1>Ceci est un footer</h1>
    <!-- <v-card flat tile class="text-center">
      <v-card-text>
        This portfolio was made with Vue.js and Vuetify. This portfolio has seen
        the light of the day on 01.20.2021, after two months learning vue.js. I
        personnaly thank everyone clicking here and seeing the evolution of my
        portfolio. As i love to say
        <q><i>we will never know everything, but we always can learn</i></q
        ><br />Thanks to Erica Grau from the Noun Project for the bear picto.
      </v-card-text>

      <v-divider></v-divider>

      <v-card-text style="color:#dc143c">
        {{ new Date().getFullYear() }} — <strong>Axelle Adamkiewicz</strong>
      </v-card-text>
    </v-card> -->
  </div></template
>
<script>
export default {
  name: "Footer",
};
</script>
